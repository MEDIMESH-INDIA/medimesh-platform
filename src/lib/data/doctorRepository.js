...
export async function searchDoctors({ filters = {}, sort = 'name_asc', offset = 0, pageSize = 12 } = {}) {
  try {
    ...
    console.warn('Canonical doctors lookup returned empty or error, falling back to starter doctors:', err?.message);
  }

  // Fallback to indexed starter doctors
  ...
  const filtered = starterDoctors.filter(doc => {
    ...
    return nameMatch && locMatch && specMatch && hospMatch;
  });
