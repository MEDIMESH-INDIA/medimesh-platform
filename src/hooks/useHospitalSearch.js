...
export function useHospitalSearch({ mode = 'canonical', filters = {}, sort = 'name_asc', pageSize = 12 }) {
  const { q = '', city = '', type = '', specialty = '', facility = '' } = filters;
  ...
  const fetchResults = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const result = await searchHospitals({ mode, filters: { q, city, type, specialty, facility }, sort, offset, pageSize });
  ...
  }, [mode, q, city, type, specialty, facility, sort, pageSize]);
